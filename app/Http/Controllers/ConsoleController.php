...
class ConsoleController extends Controller
{
    ...
    public function store(Request $request,string $module) {
        ...
        if($module==='users'){ $data=$request->validate(['name'=>'required|max:255','username'=>'required|max:100|unique:users','email'=>'nullable|email|unique:users','password'=>'required|min:8','role'=>['required',Rule::in(['admin','editor','kabupaten'])]]); $data['password']=Hash::make($data['password']); User::create($data); }
    ...
    public function update(Request $request,string $module,int $id) {
        if($module==='kelola-web'){ $this->saveSetting($request); return back()->with('success','Informasi website diperbarui.'); }
        if($module==='users'){ $item=User::findOrFail($id); $data=$request->validate(['name'=>'required|max:255','username'=>['required','max:100',Rule::unique('users')->ignore($id)],'email'=>['nullable','email',Rule::unique('users')->ignore($id)],'role'=>['required',Rule::in(['admin','editor','kabupaten'])],'is_active'=>'nullable|boolean','password'=>'nullable|min:8']); $data['is_active']=$request->boolean('is_active'); if(blank($data['password'])) unset($data['password']); else $data['password']=Hash::make($data['password']); $item->update($data); }
